...
import { Eye, Filter, Upload, Download, Search, Pencil, Trash } from "lucide-react";
import { FilePreviewModal } from "@/components/file-preview-modal";
import { FileUploadModal } from "@/components/file-upload-modal";
import { EditIncomingLetterModal } from "@/components/edit-incoming-modal";
import { DeleteConfirmationModal } from "@/components/delete-confirmation-modal";
import { PageContentSkeleton } from "@/components/page-skeleton";

// Mock data removed to prevent stale data flash
...
export function SuratMasukTable() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    const data = Array.isArray(rawData) ? rawData.map((item: any) => ({
                        ...
                        rawDate: parseDateString(item.rawDate || item.tanggal_masuk || item.tanggal || item.created_at)
                    })) : [];
    ...
    const handlePreview = (item: any) => {
        ...
        setPreviewFile({ name: item.perihal, url: url });
    ...
    const handleFileUploaded = async (file: File, url: string) => {
        ...
        try {
            ...
            setHistoryData(prev => prev.map(item =>
                item.id === selectedLetterId ? { ...item, fileUrl: url, file_url: url } : item
            ));
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100">
                            <tr>
                                ...
                                <th className="px-6 py-4">Perihal & Pengirim</th>
                                ...
                                paginatedHistory.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-slate-500 text-xs mt-0.5">{item.pengirim}</div>
                                        </td>
